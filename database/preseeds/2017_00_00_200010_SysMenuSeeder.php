...
class SysMenuSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $exception = DB::transaction(function () {
            ...
            Menu::build(function (Builder $builder) use ($id) {
                $builder->id($id)->code('sys.authority.user')->name('用户管理')->parent('sys.authority')
                    ->uri('sys.authority.user.list')->sequence($this->sequence . '0409')->tag($this->tag);
            });
            //日志
            ...
            $id = "01e86e3d7aa71ac0928e6d83c4218ad1";
            Menu::build(function (Builder $builder) use ($id) {
                $builder->id($id)->code('sys.publish.ent.token')->name('企业密钥')->parent('sys.publish')
                    ->uri('sys.publish.ent.token')
            ...
            Menu::build(function (Builder $builder) use ($id) {
                $builder->id($id)->code('sys.publish.ent')->name('发布企业')->parent('sys.publish')
                    ->uri('sys.publish.ent')
                    ->sequence($this->sequence . '0904')->tag($this->tag);
            });
        });
    }
