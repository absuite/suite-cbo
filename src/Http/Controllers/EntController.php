...
class EntController extends Controller {
  public function injection(Request $request) {
    ...
    $ent = SysModels\Ent\Ent::where('id', $input['ent_openid'])->orWhere('openid', $input['ent_openid'])->first();
    if (empty($ent)) {
    ...
    if (!$ent->hasUser($user->id)) {
      SysModels\Ent\Ent::addUser($ent->id, $user->id, ['is_effective' => 0, 'type_enum' => 'member']);
    }
