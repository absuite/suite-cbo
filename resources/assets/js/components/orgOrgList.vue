<template>
  <md-part>
    <md-part-toolbar>
      ...
      <md-part-toolbar-group>
        <md-file-import md-entity="Suite\Cbo\Models\Org"  template="/assets/vendor/suite-cbo/files/suite.cbo.org.xlsx"></md-file-import>
      </md-part-toolbar-group>
    ...
    <md-part-body class="no-padding">
      <md-query @select="select" @dblclick="edit" :md-init="initQuery" ref="list" md-query-id="suite.cbo.org.list"></md-query>
...
export default {
  ...
  methods: {
    create() {
      this.$router.push({ name: 'module', params: { module: 'cbo.org.edit' } });
    },
    edit(item) {
      this.$router.push({ name: 'id', params: { module: 'cbo.org.edit', id: item.id } });
    },
    doFetch(q) {
      if (this.currentQ != q) {
        this.currentQ = q;
    ...
    remove() {
      ...
      this.$http.delete('cbo/orgs/' + ids).then(response => {
        this.loadData();
        this.loading--;
