<template>
  <md-part>
    <md-part-toolbar>
      ...
      <md-part-toolbar-group>
        <md-file-import md-entity="Suite\Cbo\Models\Dept"  template="/assets/vendor/suite-cbo/files/suite.cbo.dept.xlsx"></md-file-import>
      </md-part-toolbar-group>
    ...
    <md-part-body class="no-padding">
      <md-query @select="select" @dblclick="edit" :md-init="initQuery" ref="list" md-query-id="suite.cbo.dept.list"></md-query>
...
export default {
  ...
  methods: {
    create() {
      this.$router.push({ name: 'module', params: { module: 'cbo.dept.edit' } });
    },
    edit(item) {
      this.$router.push({ name: 'id', params: { module: 'cbo.dept.edit', id: item.id } });
    },
    doFetch(q) {
      if (this.currentQ != q) {
        this.currentQ = q;
    ...
    remove() {
      ...
      this.$http.delete('cbo/depts/' + ids).then(response => {
        this.loadData();
        this.loading--;
