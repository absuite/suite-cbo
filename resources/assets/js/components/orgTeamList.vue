<template>
  <md-part>
    <md-part-toolbar>
      ...
      <md-part-toolbar-group>
        <md-file-import md-entity="Suite\Cbo\Models\Team"  template="/assets/vendor/suite-cbo/files/suite.cbo.team.xlsx"></md-file-import>
      </md-part-toolbar-group>
    ...
    <md-part-body class="no-padding">
      <md-query @select="select" @dblclick="edit" :md-init="initQuery" ref="list" md-query-id="suite.cbo.team.list"></md-query>
...
export default {
  ...
  methods: {
    create() {
      this.$router.push({ name: 'module', params: { module: 'cbo.team.edit' } });
    },
    edit(item) {
      this.$router.push({ name: 'id', params: { module: 'cbo.team.edit', id: item.id } });
    },
    doFetch(q) {
      if (this.currentQ != q) {
        this.currentQ = q;
